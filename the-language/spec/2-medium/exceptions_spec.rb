RSpec.describe 'exceptions' do
  ...
  it 'is possible to `rescue` an exception' do
    ...
    begin
      fail 'Oops'
    rescue StandardError => e   # => means store the error as
  ...
  it 'can also throw errors if we try editing things we are not meant to' do
    ...
    expect { cold_string << hot_string }.to raise_error(RuntimeError)
    ...
    expect { cold_array.push(hot_array) }.to raise_error(RuntimeError)
  end
end
